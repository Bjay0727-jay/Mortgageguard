...
loanRoutes.patch("/:id", requireCapability("updateLoan"), zValidator("json", updateLoanSchema), async (c) => {
  ...
  const [updated] = await sql`
    UPDATE loans SET
      ...
      application_method = COALESCE(${body.applicationMethod ?? null}, application_method),
      application_date = COALESCE(${body.applicationDate ?? null}, application_date),
      ...
      originator_nmls_id = COALESCE(${body.loanOriginatorNmlsId ?? null}, originator_nmls_id),
